...
if [[ "$(uname)" == "Linux" ]]; then
    ...
    sleep 5
    VENV_PYTHON="/home/lanforge/rat_venv/bin/python"
    ...
    if [ -f "./vrf_exec.bash" ]; then
        ...
        echo "./vrf_exec.bash not found, running python directly..."
        DISPLAY=$DISPLAY $VENV_PYTHON youtube.py --url "$URL" --host "$HOST" --device_name "$DEVICE_NAME" --duration "$DURATION" --res "$RES" $YTENV
    fi
    ...
    VENV_PYTHON="/Users/lanforge/rat_venv/bin/python"
    echo "Batch started"
    url=""
    host=""
    port=""
    ...
    export DISPLAY=:0 && $VENV_PYTHON youtube.py $args
